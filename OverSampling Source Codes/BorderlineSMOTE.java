/*  Downloaded from http://grepcode.com/file/repo1.maven.org/maven2/nz.ac.waikato.cms.weka/BorderlineSMOTE/1.0.1/weka/filters/supervised/instance/BorderlineSMOTE.java 
...
/**
 <!-- globalinfo-start -->
 * 01 Resamples a dataset by applying the Borderline Synthetic Minority Oversampling Technique (BorderlineSMOTE). The original dataset must fit entirely in memory. The amount of BorderlineSMOTE and number of nearest neighbors may be specified. For more information, see <br/>
 * <br/>
 ...
 * 
 * <pre> -P &lt;percentage&gt;
 *  Specifies percentage of BorderlineSMOTE instances to create.
 *  (default 100.0)
 * </pre>
 * 
 ...
 * 
 * <pre> -C &lt;value-index&gt;
 *  Specifies the index of the nominal class value to BorderlineSMOTE
 ...
 */
public class BorderlineSMOTE extends Filter  implements SupervisedFilter, OptionHandler, TechnicalInformationHandler {
  ...
  /** the percentage of BorderlineSMOTE instances to create. */
  ...
  public String globalInfo() {
    return "Resamples a dataset by applying theBorderline Synthetic Minority Oversampling Technique(BorderlineSMOTE)." +
    " The original dataset must fit entirely in memory." +
    " The amount of BorderlineSMOTE and number of nearest neighbors may be specified." +
  ...
  public String percentageTipText() {
    return "The percentage of BorderlineSMOTE instances to create.";
  }

  /**
   * Sets the percentage of BorderlineSMOTE instances to create.
   * 
  ...
  /**
   * Gets the percentage of BorderlineSMOTE instances to create.
   * 
   * @return 		the percentage of BorderlineSMOTE instances to create
   */
  ...
  public String classValueTipText() {
    return "The index of the class value to which BorderlineSMOTE should be applied. " +
    "Use a value of 0 to auto-detect the non-empty minority class.";
  }

  /**
   * Sets the index of the class value to which BorderlineSMOTE should be applied.
   * 
  ...
  /**
   * Gets the index of the class value to which BorderlineSMOTE should be applied.
   * 
   * @return 		the index of the clas value to which BorderlineSMOTE should be applied
   */
  ...
  public boolean batchFinished() throws Exception {
    ...
    if (!m_FirstBatchDone) {
      // Do BorderlineSMOTE, and clear the input instances.
      doBorderlineSMOTE();
	  	 // JOptionPane.showMessageDialog(null,"BorderLineSmote 19-09-2018 ");
    }
    flushInput();
  ...
  /**
   * The procedure implementing the BorderlineSMOTE algorithm. The output
   * instances are pushed onto the output queue for collection.
   * 
   * @throws Exception 	if provided options cannot be executed 
   * 			on input instances
   */
  protected void doBorderlineSMOTE() throws Exception 
  {
    int minIndex = 0;
    ...
    // the main loop to handle computing nearest neighbors and generating BorderlineSMOTE
...
for (int i = 0; i < danger.numInstances(); i++) 
	{
		Instance instanceI = danger.instance(i);
		// find k nearest neighbors for each instance
		List distanceToInstance = new LinkedList();
		if ( DangerToClass ==0)  
		{
	  ...
	  distanceToInstance.add(new Object[] {distance, instanceJ});
				...
				if ( DangerToClass ==1)  
		{
			for (int j = 0; j < Majsample.numInstances(); j++) 
			{
			Instance instanceJ = Majsample.instance(j);
			if (i != j) 
			{
				double distance = 0;
				attrEnum = getInputFormat().enumerateAttributes();
	  ...
	  distanceToInstance.add(new Object[] {distance, instanceJ});
				...
				if ( DangerToClass ==3)  
		{
			for (int j = 0; j < danger.numInstances(); j++) 
			{
			Instance instanceJ = danger.instance(j);
			if (i != j) 
			{
				double distance = 0;
				attrEnum = getInputFormat().enumerateAttributes();
      ...
      Iterator entryIterator = distanceToInstance.iterator();
      int j=0;
      while(entryIterator.hasNext() && j < knearestNeighbors) 
	  {
		nnArray[j] = (Instance) ((Object[])entryIterator.next())[1];
		j++;
      }
      // create synthetic examples
//	  int S = rand.nextInt(nearestNeighbors)+1;
//	 for ( int S = rand.nextInt(nearestNeighbors)+1; S<nearestNeighbors; S--)
//	 {	  
...
//	  }
  ...
  public static void main(String[] args) {
    runFilter(new BorderlineSMOTE(), args);
  }
}
